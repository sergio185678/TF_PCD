...
func sendToEndpoint(info Info, msg Message) {
	data := struct {
		Info    Info    `json:"info"`
	...
	if err != nil {
		fmt.Printf("Error al codificar los datos: %s\n", err)
		return
	}

	resp, err := http.Post("http://localhost:8080/data", "application/json", bytes.NewBuffer(jsonData))
	if err != nil {
		fmt.Printf("Error al enviar la solicitud HTTP: %s\n", err)
		return
	}
